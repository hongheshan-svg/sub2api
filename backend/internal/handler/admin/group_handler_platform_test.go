...
// 回归分组平台枚举:kimi/zhipu/deepseek/minimax/opencode_go/kiro 必须能通过
// Create/Update 的 binding 校验（历史 bug:调度/路由链路已支持这些平台,但 oneof
// 白名单漏加,导致平台分组无法创建、账号"无可用分组"；kiro 同理:C1 发现 oneof
// 白名单漏加 kiro,导致 Kiro 分组完全无法创建,整个 Kiro 平台不可达）;非法值
// 仍须被拒。
...
func TestGroupPlatformBinding_AllowedPlatforms(t *testing.T) {
	allowed := []string{
		...
		"kimi", "zhipu", "deepseek", "minimax", "opencode_go", "composite", "kiro",
	}
