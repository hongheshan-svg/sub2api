...
func TestBuildUpstreamTransport_OpenAIH2_EnablesPingHealthCheck(t *testing.T) {
	tr, err := buildUpstreamTransport(timeoutTestPoolSettings(), nil, upstreamProtocolModeOpenAIH2)
	require.NoError(t, err)
...
func TestBuildUpstreamTransport_OpenAIH2_WithHTTPProxy_EnablesKeepAlive(t *testing.T) {
	...
	tr, err := buildUpstreamTransport(timeoutTestPoolSettings(), proxyURL, upstreamProtocolModeOpenAIH2)
	require.NoError(t, err)
