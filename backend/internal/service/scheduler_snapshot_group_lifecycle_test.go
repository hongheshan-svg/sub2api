...
// TestSchedulerSnapshotPlatformsIncludesKiro 是 I6 的回归：
// schedulerSnapshotPlatforms 之前漏掉了 PlatformKiro，Kiro 账号的
// model_rate_limits/账号级冷却变更因此从未触发过这里驱动的调度快照失效——
// 所有调用点都是 range/`platforms[:]...` 全切片展开，不依赖数组的具体长度，
// 所以补上 Kiro 只需要这一处改动加这一条断言。
func TestSchedulerSnapshotPlatformsIncludesKiro(t *testing.T) {
	require.Contains(t, schedulerSnapshotPlatforms(), PlatformKiro)
}
