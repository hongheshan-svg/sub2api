export default {
    accounts: {
      ...
      oauth: {
        ...
        // Kiro specific
        kiro: {
          authMethodSocial: 'Social (paste refreshToken)',
          authMethodBuilderId: 'AWS Builder ID (device code)',
          authMethodIdc: 'IAM Identity Center (org SSO)',
          ...
          fieldIssuerUrl: 'SSO Portal URL',
          fieldApiKey: 'API Key',
          refreshTokenLabel: 'Refresh Token',
          refreshTokenPlaceholderSocial: 'Paste the refreshToken saved locally by the Kiro client',
          refreshTokenPlaceholderBuilderId: 'Paste a refreshToken, or use the device code authorization above',
          refreshTokenPlaceholderIdc: 'Paste a refreshToken, or use the authorization code flow above',
          refreshTokenAutoHint: 'You can also get this automatically via the device code authorization above',
          leaveEmptyHint: 'Leave blank to keep the existing value unchanged',
          regionLabel: 'Region',
          clientIdLabel: 'Client ID',
          clientSecretLabel: 'Client Secret',
          profileArnLabel: 'Profile ARN (optional)',
          profileArnHintBuilderId:
            'Tokens from Builder ID/IdC logins do not include a profileArn — only Social logins provide it automatically. Without it, account-level quota lookups and model listing may be limited. You can find it by running "kiro-cli profile" once on a machine with the Kiro CLI, or ask your organization admin.',
          profileArnHintIdc:
            'Tokens from IAM Identity Center logins do not include a profileArn. Your organization admin needs to configure a Q Developer Profile in the AWS console and provide this ARN. Without it, account-level quota lookups and model listing may be limited. You can find it by running "kiro-cli profile" once on a machine with the Kiro CLI.',
          issuerUrlLabel: 'SSO Portal URL',
          issuerUrlHint: "Your organization's IAM Identity Center (AWS SSO) portal URL",
          apiKeyLabel: 'API Key',
          fakeThinkingTitle: 'Fake Thinking (simulated)',
          fakeThinkingDesc:
            "Kiro has no native thinking output. Enabling this injects a few hundred tokens of thinking instructions into every request; the result is model-authored text, not real reasoning. Off by default.",
          // Auth wizard (authorization code / device code)
          headingIdc: 'IAM Identity Center Authorization Code',
          headingBuilderId: 'AWS Builder ID Device Code',
          idleDescIdc: 'This opens the AWS SSO login page in a new tab — sign in with your organization account and complete authorization.',
          idleDescBuilderId: 'This generates a one-time device code — sign in with your AWS Builder ID account on the page that opens and approve it.',
          startButtonLoading: 'Fetching…',
          startButtonIdc: 'Generate Authorization Link',
          startButtonBuilderId: 'Get Device Code',
          issuerUrlRequiredHint: 'Please fill in the SSO Portal URL below first',
          startedIdcOpenedHint: 'The authorization link was opened in a new tab. If it was blocked,',
          startedIdcReopenLink: 'click here to reopen it',
          startedIdcInstructions:
            'After signing in and approving, the browser will redirect to an address that fails to load ("unable to connect" — this is expected, nothing is listening there). Copy the complete URL from the address bar and paste it below, then submit.',
          expiresInSeconds: 'Authorization link expires in {seconds}s',
          sessionExpired: 'The authorization session has expired — please generate a new link',
          submitButtonLoading: 'Submitting…',
          submitCallbackUrl: 'Submit Callback URL',
          restartButton: 'Restart',
          userCodeCopied: 'Copied',
          userCodeCopy: 'Copy',
          builderIdOpenedHint: 'The verification page was opened in a new tab and pre-filled with this code. If it did not open automatically,',
          builderIdOpenLink: 'click here to open it',
          deviceCodeExpiresInSeconds: 'Device code expires in {seconds}s — polling for authorization result…',
          deviceCodeExpired: 'The device code has expired — please request a new one',
          reacquireButton: 'Get New Code',
          successMessage: 'Credentials retrieved — the fields below have been filled in automatically. Reauthorize to switch accounts.',
          reauthorizeButton: 'Reauthorize',
          errorMissingIssuerUrl: 'Please fill in the SSO Portal URL first',
          errorGenerateAuthUrlFailed: 'Failed to generate the authorization link',
          errorMissingCallbackUrl: 'Please paste the full URL from the browser address bar after authorizing',
          errorParseCallbackFailed: 'Could not parse an authorization result from the pasted URL — make sure you copied the complete address bar URL',
          errorCompleteAuthFailed: 'Failed to complete authorization',
          errorGetDeviceCodeFailed: 'Failed to get the device code',
          errorDeviceAuthFailed: 'Device code authorization failed'
        },
        // Gemini specific
	        gemini: {
