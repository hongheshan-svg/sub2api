...
interface NavItem {
  ...
  /**
   * 外部链接 URL。设置后菜单项渲染为 <a target="_blank">，不参与路由。
   * `path` 仍作为稳定 key 使用。
   */
  externalUrl?: string
}
...
// const ImageIcon = { ... } // hidden: image generation menu removed from user sidebar
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/invoice', label: t('nav.invoiceManagement'), icon: OrderListIcon },
    // { path: '/image-generation', label: t('nav.imageGeneration'), icon: ImageIcon },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/invoices', label: t('nav.adminInvoices'), icon: OrderIcon },
